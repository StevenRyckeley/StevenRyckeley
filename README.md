# Hi, I'm Steven Ryckeley 👋

### Aspiring Cybersecurity Professional | BBA Information Systems | University of Texas at Arlington

---

# 🧑‍💻 About Me

---

I am a senior Information Systems student at the University of Texas at Arlington, graduating in May 2026, with a strong foundation in programming, data analysis, and business operations. My academic background has equipped me with technical proficiency in Python, Java, and SQL, as well as a comprehensive understanding of how technology supports and secures organizational objectives.

My interest in cybersecurity stems from a deep commitment to protecting individuals and organizations from the ever-evolving landscape of digital threats. I am drawn to the analytical and problem-solving nature of the field — the need to think critically, anticipate vulnerabilities, and respond decisively. I am currently pursuing the Google Cybersecurity Professional Certificate, where I am developing hands-on skills in network security, SIEM tools, incident detection and response, and risk management frameworks such as NIST.

I bring a unique perspective to cybersecurity by combining a technical skill set with a business-oriented mindset. My ability to understand both the operational and technological dimensions of an organization allows me to assess security risks not just in isolation, but in the context of broader business impact. I am committed to continuous learning, evidenced by my concurrent pursuit of the Python Institute PCEP certification, with plans to advance to the PCAP and the Microsoft Cybersecurity Analyst Professional Certificate.

I am actively seeking an entry-level role as an IT Security Analyst or Cybersecurity Analyst in the DFW area, where I can contribute to a security-focused team, apply my growing skill set, and continue developing as a cybersecurity professional.

---

## 🛡️ Cybersecurity Focus Areas

- Network Security & Traffic Analysis
- Security Information & Event Management (SIEM)
- Incident Detection & Response
- Risk Management & Security Frameworks (NIST, CIA Triad)
- Linux & Command Line Operations
- SQL for Security Log Filtering & Investigation

---

## 🧰 Technical Skills

| Category | Tools & Technologies |
|---|---|
| **Languages** | Python, Java, SQL |
| **Security Tools** | Wireshark, tcpdump, Chronicle, Splunk |
| **Operating Systems** | Windows, Linux (CLI) |
| **Frameworks** | NIST Cybersecurity Framework, NIST RMF |
| **IDE** | Visual Studio Code |
| **Other** | Google Workspace, Microsoft Office Suite |

---

## 📂 Featured Projects

> *Projects are being added as I complete labs and coursework. Check back often!*

| Project | Description | Tools Used |
|---|---|---|
| 🔍 [Google Cybersecurity Labs](https://github.com/StevenRyckeley/google-cybersecurity-cert) | Hands-on labs and write-ups from the Google Cybersecurity Professional Certificate | Python, SQL, Linux, Wireshark |
| 🐍 [Python Security Scripts](https://github.com/StevenRyckeley/python-security-scripts) | Python scripts for automating security tasks and log analysis | Python |
| 📋 [Incident Response Journal](https://github.com/StevenRyckeley/incident-response-journal) | Documented incident response scenarios based on real-world case studies | Markdown, NIST Framework |

---

## 📜 Certifications

| Status | Certification |
|---|---|
| ✅ Complete | **Google Cybersecurity Professional Certificate** - Coursera (2026) |
| 🟡 In Progress | **Python Institute PCEP** — Certified Entry-Level Python Programmer |
| 🔲 Planned | **Microsoft Cybersecurity Analyst Professional Certificate** |
| 🔲 Planned | **Python Institute PCAP** — Certified Associate in Python Programming |

---

## 🎓 Education

**Bachelor of Business Administration — Information Systems**
University of Texas at Arlington | *Expected Graduation: May 2026*

*Relevant Coursework:* Business Analytics, Cybersecurity, Operations Management, Strategic Management, Introduction to Programming (Python), Advanced Application Development (Java), Database Management Systems (SQL)

---

## 📫 Let's Connect

<a href="https://www.linkedin.com/in/steven-ryckeley/">
  <img src="https://img.shields.io/badge/LinkedIn-Steven%20Ryckeley-blue?style=flat&logo=linkedin" alt="LinkedIn"/>
</a>
<a href="mailto:saryck01@gmail.com">
  <img src="https://img.shields.io/badge/Email-Contact%20Me-red?style=flat&logo=gmail" alt="Email"/>
</a>

---

*"Security is not a product, but a process." — Bruce Schneier*
